...
A lookup table, not a precondition: load the matching skill when you are doing
something in TanStack Router or Start that you would otherwise guess at, and
skip it for routine edits.

This block was generated by TanStack Intent and then moved here out of
`AGENTS.md`, which is loaded on every request and does not need 100 lines of
index in it. Generation is off (`"intent": false` in `.cta.json`), so the list
no longer updates itself — to refresh it, flip that flag back to `true`, let
the tool rewrite `AGENTS.md`, and move the new block back here.
